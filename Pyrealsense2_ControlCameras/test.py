...
Keyboard:
    [p]     Pause
    [r]     Reset View
    [d]     Cycle through decimation values    循环通过抽取值
    [z]     Toggle point scaling               切换点缩放
    [x]     Toggle point distance attenuation  切换点距离衰减
    [c]     Toggle color source                切换颜色来源
    [l]     Toggle lighting                    切换照明
    [f]     Toggle depth post-processing       切换深度后处理
    [s]     Save PNG (./out.png)               保存 PNG (./out.png)
    [e]     Export points to ply (./out.ply)   将点导出到层 (./out.ply)
    [q/ESC] Quit                               [q/ESC] 退出
...
def grid(size=1, n=10, width=1):
    """draw a grid on xz plane  在 xz 平面上绘制网格 """
    gl.glLineWidth(width)
    s = size / float(n)
    s2 = 0.5 * size
    batch = pyglet.graphics.Batch()
...
def run(dt):
    global w, h
    global i
    window.set_caption("RealSense (%dx%d) %3d FPS (%.2fms) %s" %
    ...
    if keys[pyglet.window.key.E]:
        start = time.time()
        # points.export_to_ply("./out%d.ply" % i, mapped_frame)
